...
public interface InterScheduler extends Nameable, DatacenterEntity {
    ...
    /**
     * 获取一批实例组进行预处理后，放入 waitSchedulingInstanceGroupQueue 中
     * Get a batch of instance groups from the instance group queue for prescheduling.
     * It will call different prescheduling functions according to the waitSchedulingInstanceGroupQueue type.
     */
    void presolve();

    /**
     * Get the time spent on the presolving.
     * @return the time spent on the presolving.
     */
    double getPresolveTime();

    /**
     * Get whether the pipeline is enabled.
     * @return whether the pipeline is enabled.
     */
    boolean isEnablePipeline();

    /**
     * Set whether the pipeline is enabled.
     * @param enablePipelineFlag whether the pipeline is enabled.
     * @return the inter-scheduler itself.
     */
    InterScheduler setEnablePipelineFlag(boolean enablePipelineFlag);
    ...
    /**
     * Set the state synchronization competition interval of each data center.
     * 
     * @param dcStateSynCompeteInterval the state synchronization competition interval of each data center.
     * @return the inter-scheduler itself.
     */
    InterScheduler setDcStateSynCompeteInterval(Map<Datacenter, Double> dcStateSynCompeteInterval);
    ...
    /**
     * Set the instance group queue for new instance groups.
     * @param instanceGroupQueue the instance group queue for new instance groups.
     * @return the inter-scheduler itself.
     */
    InterScheduler setInstanceGroupQueue(InstanceGroupQueue instanceGroupQueue);

    /**
     * Set the instance group queue for retry instance groups.
     * @param instanceGroupQueue the instance group queue for retry instance groups.
     * @return the inter-scheduler itself.
     */
    InterScheduler setRetryInstanceGroupQueue(InstanceGroupQueue instanceGroupQueue);
    ...
    /**
     * Get whether the presolve queues are empty.
     * @return whether the presolve queues are empty.
     */
    boolean isPresolveQueuesEmpty();

    /**
     * Get whether the schedule queues are empty.
     * @return whether the schedule queues are empty.
     */
    boolean isScheduleQueuesEmpty();
