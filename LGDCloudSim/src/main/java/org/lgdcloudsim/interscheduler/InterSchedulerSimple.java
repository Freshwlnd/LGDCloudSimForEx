...
public class InterSchedulerSimple implements InterScheduler {
    ...
    public InterScheduler setInstanceGroupQueue(InstanceGroupQueue instanceGroupQueue) {
        this.instanceGroupQueue = instanceGroupQueue;
        return this;
    }
    ...
    public InterScheduler setRetryInstanceGroupQueue(InstanceGroupQueue retryInstanceGroupQueue) {
        this.retryInstanceGroupQueue = retryInstanceGroupQueue;
        return this;
    }
    
    /**
     * The instance group queue that stores the instance groups that need to be scheduled.
     * It is used to store the instance groups that need to be scheduled in the current round of scheduling.
     * The instance group will be fetched from the instanceGroupQueue and the retryInstanceGroupQueue.
     */
    @Getter
    @Setter
    // InstanceGroupQueue waitSchedulingInstanceGroupQueue = new InstanceGroupQueueCFS();
    InstanceGroupQueue waitSchedulingInstanceGroupQueue = new InstanceGroupQueueFifo();

    public InterScheduler setwaitSchedulingInstanceGroupQueue(InstanceGroupQueue waitSchedulingInstanceGroupQueue) {
        this.waitSchedulingInstanceGroupQueue = waitSchedulingInstanceGroupQueue;
        return this;
    }
    ...
    @Getter
    @Setter
    protected Map<Datacenter, String> dcStateSynType = new HashMap<>();

    /**
     * The state synchronization competition interval of each data center.
     */
    @Getter
    @Setter
    private Map<Datacenter, Double> DcStateSynCompeteInterval = new HashMap<>();
    ...
    double excludeTime = 0.0;

    /**
     * The time spent on the presolve.
     */
    @Getter
    double presolveTime = 0.0;
    ...
    /**
     * The flag to set whether the pipeline is enabled.
     **/
    @Getter
    @Setter
    private boolean enablePipelineFlag = false;

    public boolean isEnablePipeline() {
        return enablePipelineFlag;
    }
    ...
    public void presolve() {
        // 获取所有待调度请求，预处理后放至 waitSchedulingInstanceGroupQueue 中

        QueueResult<InstanceGroup> queueResult = getUnpresolvedInstanceGroups();
        List<InstanceGroup> waitPresolveInstanceGroups = queueResult.getWaitScheduledItems();
        
        waitSchedulingInstanceGroupQueue.add(waitPresolveInstanceGroups);

        this.presolveTime = waitSchedulingInstanceGroupQueue.getSortTime();

        setInstanceGroupInterSchedulePresolveEndTime(waitPresolveInstanceGroups, getSimulation().clock() + this.presolveTime);
    }

    /**
     * Get the instance groups that need to be presolved from the instance group queues.
     * @return the result of the instance group queues
     */
    private QueueResult<InstanceGroup> getUnpresolvedInstanceGroups() {
    ...
    private void setInstanceGroupInterSchedulePresolveEndTime(List<InstanceGroup> afterPresolvingInstanceGroups, double presolveEndTime) {
        for (InstanceGroup instanceGroup : afterPresolvingInstanceGroups) {
            instanceGroup.setInterSchedulePresolveTime(presolveEndTime);
        }
    }
    ...
    public InterSchedulerResult schedule() {
        ...
        this.scheduleTime = Math.max(0.1, end - start - this.excludeTime);
    ...
    private QueueResult<InstanceGroup> getWaitSchedulingInstanceGroups() {
        double nowTime = getSimulation().clock();
        // if (!waitSchedulingInstanceGroupQueue.isEmpty()) {   // if there are instance groups that have been preprocessed
            return waitSchedulingInstanceGroupQueue.getBatchItem(nowTime);
        // } 
        // else if (retryInstanceGroupQueue.isEmpty()) {
        //     return instanceGroupQueue.getBatchItem(nowTime);
        // } else {
        //     QueueResult<InstanceGroup> queueResult = retryInstanceGroupQueue.getBatchItem(nowTime);
    ...
    public boolean isQueuesEmpty() {
        return instanceGroupQueue.isEmpty() && retryInstanceGroupQueue.isEmpty() && waitSchedulingInstanceGroupQueue.isEmpty();
    }

    @Override
    public boolean isPresolveQueuesEmpty() {
        return instanceGroupQueue.isEmpty() && retryInstanceGroupQueue.isEmpty();
    }

    @Override
    public boolean isScheduleQueuesEmpty() {
        return waitSchedulingInstanceGroupQueue.isEmpty();
    }
