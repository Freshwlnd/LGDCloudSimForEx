...
public class DatacenterSimple extends CloudSimEntity implements Datacenter {
    ...
    /**
     * The flag to set whether the datacenter is a failure cluster.
     **/
    @Setter
    private boolean failureClusterFlag = false;
    ...
    /**
     * Whether the interScheduler is busy in presolve stage.
     **/
    private Map<InterScheduler, Boolean> isInterSchedulerPresolveBusy = new HashMap<>();

    /**
     * Whether the interScheduler is busy in schedule stage.
     **/
    private Map<InterScheduler, Boolean> isInterSchedulerScheduleBusy = new HashMap<>();
    ...
    public DatacenterSimple(@NonNull Simulation simulation) {
        ...
        this.instanceQueue = new InstanceQueueFifo(this);
...
//                LOGGER.info("{}: {} received {} instances to finish", getSimulation().clockStr(), getName(), list.size());
        ...
        if(!this.isFailureCluster()) {
            if (getSimulation().clock() - instance.getStartTime() >= instance.getLifecycle() - 0.01 && instance.getLifecycle() != -1) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.warn("{}: {}'s Instance{} successfully completed running on host{} and resources have been released", getSimulation().clockStr(), getName(), instance.getId(), hostId);
                }
            } else {
    ...
    private void updateInstanceGroupAfterSuccess(InstanceGroup instanceGroup) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.warn("{}: {}'s InstanceGroup{} successfully completed running.", getSimulation().clockStr(), getName(), instanceGroup.getId());
        }
    ...
    private void updateUserRequestAfterSuccess(UserRequest userRequest) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.warn("{}: userRequest{} successfully completed running.", getSimulation().clockStr(), getName());
        }
    ...
    private Map<IntraScheduler, List<Instance>> allocateResource(Map<IntraScheduler, List<Instance>> allocatedResult) {
        ...
        for (Map.Entry<IntraScheduler, List<Instance>> entry : allocatedResult.entrySet()) {
            ...
            for (Instance instance : instances) {
                ...
                if (!this.isFailureCluster() && instance.getUserRequest().getState() == UserRequest.FAILED) {
                    continue;
                }
    ...
    private void intraScheduleFailed(List<Instance> instances, IntraScheduler intraScheduler, boolean isNeedRevertSelfHostState, Set<UserRequest> outDatedUserRequests) {
        ...
        while (instanceIterator.hasNext()) {
            ...
            if ((this.isFailureCluster() && !instance.canRetry()) || (!this.isFailureCluster() && instance.isFailed())) {
                if(LOGGER.isDebugEnabled() && this.isFailureCluster()) {
                    LOGGER.error("{}: {}'s {} failed to schedule instance{}", getSimulation().clockStr(), getName(), intraScheduler.getName(), instance.getId());
                }
        ...
        if (!failedUserRequests.isEmpty()) {
            if(this.isFailureCluster()) {
                LOGGER.error("{}: {}'s {} failed to schedule {} instances", getSimulation().clockStr(), getName(), intraScheduler.getName(), failedUserRequests.size());
            } else {
    ...
    private void intraLoadBalance() {
        ...
        if (!instances.isEmpty()) {
            ...
            if(this.isFailureCluster()) {
                intraLoadBalancer.setLoadBalanceCostTime(0);
            }
            ...
            for (Map.Entry<InterScheduler, List<InstanceGroup>> entry : loadBalanceResult.entrySet()) {
                ...
                if(!interScheduler.isEnablePipeline()) {
                    if ((!isInterSchedulerBusy.containsKey(interScheduler) || !isInterSchedulerBusy.get(interScheduler))
                        ...
                        send(this, interLoadBalancer.getLoadBalanceCostTime(), CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
                        isInterSchedulerBusy.put(interScheduler, true);
                    }
                } else {
                    if((!isInterSchedulerPresolveBusy.containsKey(interScheduler) || !isInterSchedulerPresolveBusy.get(interScheduler))
                            && !interScheduler.isPresolveQueuesEmpty()) {
                        send(this, interLoadBalancer.getLoadBalanceCostTime(), CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
                        isInterSchedulerPresolveBusy.put(interScheduler, true);
                    }
                }
        ...
        if (evt.getData() instanceof InterScheduler interScheduler) {
            ...
            if(!interScheduler.isEnablePipeline()) {
                ...
                if(!interScheduler.isPresolveQueuesEmpty()) {
                    ...
                    isInterSchedulerPresolveBusy.put(interScheduler, false);
                }
                if ((!isInterSchedulerScheduleBusy.containsKey(interScheduler) || !isInterSchedulerScheduleBusy.get(interScheduler))
                            && !interScheduler.isScheduleQueuesEmpty()) {
                    send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
                    isInterSchedulerScheduleBusy.put(interScheduler, true);
                }
            }
        }
    }
    ...
    private void processScheduleToDcHostResponse(SimEvent evt) {
        if (evt.getTag() == CloudSimTag.SCHEDULE_TO_DC_HOST_CONFLICTED) {
            if (evt.getData() instanceof InterSchedulerSendItem sendItem) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.warn("{}: {}'s {} failed to schedule {} instanceGroups,it need retry soon.", getSimulation().clockStr(), getName(), interScheduler.getName(), failedOutdatedResult.getFailRes().size());
                }
            }
        }
    }
    ...
    private void processUserRequestsSend(final SimEvent evt) {
        if (evt.getData() instanceof List<?> userRequestsTmp) {
            ...
            if (this.isFailureCluster()) {
                acceptUserRequestForFailureCluster((List<UserRequest>)userRequestsTmp);
            ...
            } else if(getSimulation().isSingleDatacenterFlag()){
    ...
    /**
     * When experimenting in a failure cluster scenario, user requests are accepted.
     * @param userRequests the user requests need to be accepted
     */
    private void acceptUserRequestForFailureCluster(List<UserRequest> userRequests){
    ...
    private void processInterScheduleBegin(final SimEvent evt) {
        if (evt.getData() instanceof InterScheduler interScheduler) {
            ...
            interSchedulerResult = interScheduler.schedule();
    ...
    private void startInterScheduling(InterScheduler interScheduler) {
        if(!interScheduler.isEnablePipeline()) {
            ...
            if(interScheduler.isScheduleQueuesEmpty()) {
                isInterSchedulerScheduleBusy.put(interScheduler, false);
            } else {
                send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
            }
        }
    ...
    @Override
    public boolean isFailureCluster() {
        return failureClusterFlag;
    }
