...
public class InitDatacenter {
    ...
    public static void initDatacenters(Simulation LGDCloudSim, Factory factory, String filePath) {
        ...
        if (jsonObject.containsKey("failureCluster")) {
            initFailureCluster(jsonObject);
        }
    }
    ...
    private static InterScheduler initInterScheduler(String type, JsonObject interSchedulerJson, int collaborationId, CollaborationManager collaborationManager) {
        ...
        if(interSchedulerJson.containsKey("enablePipeline")) {
            interScheduler.setEnablePipelineFlag(interSchedulerJson.getBoolean("enablePipeline"));
        }

        Object[] dcStateSynInfo = getDcStateSynInfo(interSchedulerJson, collaborationManager);
        interScheduler.setDcStateSynInterval((Map<Datacenter, Double>) dcStateSynInfo[0]);
        interScheduler.setDcStateSynType((Map<Datacenter, String>) dcStateSynInfo[1]);
     ...
     */
    private static Object[] getDcStateSynInfo(JsonObject dcStateSynInfoJson, CollaborationManager collaborationManager) {
    ...
    /**
     * Initialize the failure cluster.
     *
     * @param jsonObject the json object of the failure cluster
     */
    private static void initFailureCluster(JsonObject jsonObject) {
        // config failureClusters parameters
        JsonObject rawFailureClusterJson = jsonObject.getJsonObject("failureCluster");
        JsonBuilderFactory factory = Json.createBuilderFactory(null);
        JsonObjectBuilder failureClusterJson = Json.createObjectBuilder(rawFailureClusterJson);

        Boolean isOnlyOneHost = true;

        // state manager
        Integer sumHostNum = 0;
        Integer minCpu = 1024;
        Integer minRam = 2048;
        Integer minStorage = 12800;
        Integer minBw = 6400;
        Integer sumCpu = 0;
        Integer sumRam = 0;
        Integer sumStorage = 0;
        Integer sumBw = 0;
        Double maxPricePerCPUPerSec = 1.0;
        Double maxPricePerCPU = 1.0;
        Double maxPricePerRamPerSec = 1.0;
        Double maxPricePerRam = 1.0;
        Double maxPricePerStoragePerSec = 1.0;
        Double maxPricePerStorage = 1.0;
        Double maxPricePerBwPerSec = 1.0;
        Double maxPricePerBw = 1.0;
        Double maxPricePerRack = 0.1;
        Double minHostPerRack = 100.;

        Double MU = 2.0;
        try {
            MU = rawFailureClusterJson.getJsonNumber("MU").doubleValue();
        } catch (Exception e) {
            LOGGER.info("mu is not set, use the default value 2.0");
        }
        ...
        if (jsonObject.containsKey("collaborations")) {
            ...
            for (int i = 0; i < jsonObject.getJsonArray("collaborations").size(); i++) {
                ...
                for (int j = 0; j < collaborationJson.getJsonArray("datacenters").size(); j++) {
                    ...
                    sumHostNum += datacenterJson.getInt("hostNum");
                    ...
                    for (int k = 0; k < datacenterJson.getJsonArray("hostStates").size(); k++) {
                        ...
                        minCpu = Math.min(minCpu, hostStateJson.getInt("cpu"));
                        minRam = Math.min(minRam, hostStateJson.getInt("ram"));
                        minStorage = Math.min(minStorage, hostStateJson.getInt("storage"));
                        minBw = Math.min(minBw, hostStateJson.getInt("bw"));
                        sumCpu += hostStateJson.getInt("cpu");
                        sumRam += hostStateJson.getInt("ram");
                        sumStorage += hostStateJson.getInt("storage");
                        sumBw += hostStateJson.getInt("bw");
                    }

                    JsonObject unitPriceJson = datacenterJson.getJsonObject("resourceUnitPrice");
                    if(unitPriceJson.containsKey("pricePerCpuPerSec")) {
                        maxPricePerCPUPerSec = Math.max(maxPricePerCPUPerSec, unitPriceJson.getJsonNumber("pricePerCpuPerSec").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerCpu")) {
                        maxPricePerCPU = Math.max(maxPricePerCPU, unitPriceJson.getJsonNumber("pricePerCpu").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerRamPerSec")) {
                        maxPricePerRamPerSec = Math.max(maxPricePerRamPerSec, unitPriceJson.getJsonNumber("pricePerRamPerSec").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerRam")) {
                        maxPricePerRam = Math.max(maxPricePerRam, unitPriceJson.getJsonNumber("pricePerRam").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerStoragePerSec")) {
                        maxPricePerStoragePerSec = Math.max(maxPricePerStoragePerSec, unitPriceJson.getJsonNumber("pricePerStoragePerSec").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerStorage")) {
                        maxPricePerStorage = Math.max(maxPricePerStorage, unitPriceJson.getJsonNumber("pricePerStorage").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerBwPerSec")) {
                        maxPricePerBwPerSec = Math.max(maxPricePerBwPerSec, unitPriceJson.getJsonNumber("pricePerBwPerSec").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerBw")) {
                        maxPricePerBw = Math.max(maxPricePerBw, unitPriceJson.getJsonNumber("pricePerBw").doubleValue());
                    }
                    if(unitPriceJson.containsKey("pricePerRack")) {
                        maxPricePerRack = Math.max(maxPricePerRack, unitPriceJson.getJsonNumber("pricePerRack").doubleValue());
                    }
                    if(unitPriceJson.containsKey("HostPerRack")) {
                        minHostPerRack = Math.min(minHostPerRack, unitPriceJson.getJsonNumber("HostPerRack").doubleValue());
                    }
                }
            }
        } else {
            // SingleDatacenter
            JsonObject datacenterJson = jsonObject;
            sumHostNum += datacenterJson.getInt("hostNum");
            for (int k = 0; k < datacenterJson.getJsonArray("hostStates").size(); k++) {
                JsonObject hostStateJson = datacenterJson.getJsonArray("hostStates").getJsonObject(k);
                minCpu = Math.min(minCpu, hostStateJson.getInt("cpu"));
        ...
        if (isOnlyOneHost) {
            sumHostNum = 1;
            sumCpu = (int) (MU * sumCpu);
            sumRam = (int) (MU * sumRam);
            sumStorage = (int) (MU * sumStorage);
            sumBw = (int) (MU * sumBw);
        } else {
            Double hostNum_double = MU * sumHostNum;
            sumHostNum = hostNum_double.intValue();
        }

        if(!rawFailureClusterJson.containsKey("hostNum")) {
            failureClusterJson.add("hostNum", sumHostNum);
        }
        
        JsonArrayBuilder partitions = factory.createArrayBuilder();
        JsonObjectBuilder partition = factory.createObjectBuilder();
        partition.add("length", sumHostNum);
        partitions.add(partition.build());
        if(!rawFailureClusterJson.containsKey("partitions")) {
            failureClusterJson.add("partitions", partitions.build());
        }
        if(!rawFailureClusterJson.containsKey("synchronizationGap")) {
            failureClusterJson.add("synchronizationGap", 0.);
        }

        // config hostStates parameters
        JsonArrayBuilder hostStates = factory.createArrayBuilder();
        if (isOnlyOneHost) {
            JsonObjectBuilder hostState = factory.createObjectBuilder();
            hostState.add("cpu", sumCpu);
            hostState.add("ram", sumRam);
            hostState.add("storage", sumStorage);
            hostState.add("bw", sumBw);
            hostState.add("startId", 0);
            hostState.add("length", sumHostNum);
            hostStates.add(hostState.build());
        } else {
            JsonObjectBuilder hostState = factory.createObjectBuilder();
            hostState.add("cpu", minCpu);
            hostState.add("ram", minRam);
            hostState.add("storage", minStorage);
            hostState.add("bw", minBw);
            hostState.add("startId", 0);
            hostState.add("length", sumHostNum);
            hostStates.add(hostState.build());
        }
        // config hostStates parameters end

        if(!rawFailureClusterJson.containsKey("hostStates")) {
            failureClusterJson.add("hostStates", hostStates.build());
        }
        JsonArrayBuilder intraSchedulers = factory.createArrayBuilder();
        JsonObjectBuilder intraScheduler = factory.createObjectBuilder();
        intraScheduler.add("type", "random");
        intraSchedulers.add(intraScheduler.build());
        if(!rawFailureClusterJson.containsKey("intraSchedulers")) {
            failureClusterJson.add("intraSchedulers", intraSchedulers.build());
        }
        JsonObjectBuilder intraLoadBalancer = factory.createObjectBuilder();
        intraLoadBalancer.add("type", "batch");
        if(!rawFailureClusterJson.containsKey("intraLoadBalancer")) {
            failureClusterJson.add("intraLoadBalancer", intraLoadBalancer.build());
        }
        JsonObjectBuilder resourceAllocateSelector = factory.createObjectBuilder();
        resourceAllocateSelector.add("type", "simple");
        if(!rawFailureClusterJson.containsKey("resourceAllocateSelector")) {
            failureClusterJson.add("resourceAllocateSelector", resourceAllocateSelector.build());
        }
        JsonObjectBuilder resourceUnitPrice = factory.createObjectBuilder();
        resourceUnitPrice.add("pricePerCpuPerSec", MU * maxPricePerCPUPerSec);
        resourceUnitPrice.add("pricePerCpu", MU * maxPricePerCPU);
        resourceUnitPrice.add("pricePerRamPerSec", MU * maxPricePerRamPerSec);
        resourceUnitPrice.add("pricePerRam", MU * maxPricePerRam);
        resourceUnitPrice.add("pricePerStoragePerSec", MU * maxPricePerStoragePerSec);
        resourceUnitPrice.add("pricePerStorage", MU * maxPricePerStorage);
        resourceUnitPrice.add("pricePerBwPerSec", MU * maxPricePerBwPerSec);
        resourceUnitPrice.add("pricePerBw", MU * maxPricePerBw);
        resourceUnitPrice.add("PricePerRack", MU * maxPricePerRack);
        resourceUnitPrice.add("HostPerRack", 1.0/MU * minHostPerRack);
        if(!rawFailureClusterJson.containsKey("resourceUnitPrice")) {
            failureClusterJson.add("resourceUnitPrice", resourceUnitPrice.build());
        }
        // config failureClusters parameters end

        Datacenter datacenter = getDatacenter(failureClusterJson.build(), 0, false, InterSchedulerSimple.NULL, false);
        datacenter.setFailureClusterFlag(true);
        datacenter.setName("FailureCluster");
        CloudInformationService cloudInformationService = LGDCloudSim.getCis();
        cloudInformationService.setFailureCluster(datacenter);
    }
    ...
    private static StatesManager getStatesManager(JsonObject datacenterJson, boolean isCenterSchedule, int target) {
        ...
        if (datacenterJson.containsKey("competeGap")) {
            double competeGap = datacenterJson.getJsonNumber("competeGap").doubleValue();
            statesManager.setCompeteGap(competeGap);
        }
