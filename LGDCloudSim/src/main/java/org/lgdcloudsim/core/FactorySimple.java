...
import org.lgdcloudsim.conflicthandler.*;
...
public class FactorySimple implements Factory {
    ...
    public InterScheduler getInterScheduler(String type, int id, Simulation simulation, int collaborationId, int target, boolean isSupportForward) {
        return switch (type) {
            ...
            case "Lattice", "lattice" -> new InterSchedulerLattice(id, simulation, collaborationId, target, isSupportForward);
            case "Diktyo", "diktyo" -> new InterSchedulerDiktyo(id, simulation, collaborationId, target, isSupportForward);
            case "Tango", "tango", "TanGo", "tanGo" -> new InterSchedulerTango(id, simulation, collaborationId, target, isSupportForward);
            case "DelayFirst", "Delayfirst", "delayfirst", "delayFirst", "Delay-first", "Delay-First" -> new InterSchedulerDelayFirst(id, simulation, collaborationId, target, isSupportForward);
            default -> null;
        };
    }
    ...
    @Override
    public SqlRecord getSqlRecord(String type, String dbName, String dirSuf) {
        return switch (type) {
            case "simple", "Simple" -> new SqlRecordSimple(dbName, dirSuf);
            case "detailscheduletime", "detailScheduleTime", "DetailScheduleTime" -> new SqlRecordDetailScheduleTime(dbName, dirSuf);
            case "Null", "NULL", "null" -> new SqlRecordNull();
            default -> null;
        };
    }
