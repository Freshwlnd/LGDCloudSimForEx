...
public class CloudSimTag {
    ...
    /**
     * Denotes the start of inter-scheduler presolving.
     * Note that it must be bigger than {@link #USER_REQUEST_SEND}.
     * Therefore, when two events arrive at the same time,
     * the user request can be placed in the queue first, and then inter-scheduler presolving can be started.
     */
    public static final int INTER_PRESOLVE_BEGIN = BASE + 5;

    /**
     * Denotes the end of inter-scheduler presolving.
     * The event will be sent after the execution of the {@link #INTER_PRESOLVE_BEGIN}.
     * The time interval between two events is the time spent on inter-scheduler presolving.
     */
    public static final int INTER_PRESOLVE_END = INTER_PRESOLVE_BEGIN + 1;
     ...
     */
    public static final int INTER_SCHEDULE_BEGIN = INTER_PRESOLVE_END + 1;
    ...
    public static String tagToString(int tag) {
        return switch (tag) {
            ...
            case INTER_PRESOLVE_BEGIN -> "INTER_PRESOLVE_BEGIN";
            case INTER_PRESOLVE_END -> "INTER_PRESOLVE_END";
