...
public class CloudInformationService extends CloudSimEntity {
    ...
    /**
     * The virtual data center that is used to record the failed user requests.
     */
    @Getter
    @Setter
    private Datacenter failureCluster;
    ...
    public CloudInformationService(@NonNull Simulation simulation) {
        ...
        this.failureCluster = Datacenter.NULL;
    }
    ...
    public void processEvent(SimEvent evt) {
        switch (evt.getTag()) {
            ...
            case CloudSimTag.DC_REGISTRATION_REQUEST -> updateDatacenterList(evt);
            ...
            case CloudSimTag.INTER_PRESOLVE_BEGIN -> processInterPresolveBegin(evt);
            case CloudSimTag.INTER_PRESOLVE_END -> processInterPresolveEnd(evt);
    ...
    /**
     * Updates the list of data centers in the system.
     * @param evt the event
     */
    private void updateDatacenterList(SimEvent evt) {
        Datacenter dc = (Datacenter) evt.getData();
    ...
    private void processLoadBalanceSend(SimEvent evt) {
        if (evt.getData() instanceof Integer collaborationId) {
            ...
            Map<Integer, Boolean> centerSchedulersPresolveBusyMap = collaborationManager.getCenterSchedulersPresolveBusyMap(collaborationId);
            ...
            for (Map.Entry<InterScheduler, List<InstanceGroup>> entry : loadBalanceResult.entrySet()) {
                ...
                if(!interScheduler.isEnablePipeline()) {
                    if ((!centerSchedulersBusyMap.containsKey(interScheduler.getId()) || !centerSchedulersBusyMap.get(interScheduler.getId()))
                        ...
                        send(this, loadBalancer.getLoadBalanceCostTime(), CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
                        centerSchedulersBusyMap.put(interScheduler.getId(), true);
                    }
                } else {
                    if((!centerSchedulersPresolveBusyMap.containsKey(interScheduler.getId()) || !centerSchedulersPresolveBusyMap.get(interScheduler.getId()))
                            && !interScheduler.isPresolveQueuesEmpty()) {
                        send(this, loadBalancer.getLoadBalanceCostTime(), CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
                        centerSchedulersPresolveBusyMap.put(interScheduler.getId(), true);
                    }
                }
                ...
                //         && !interScheduler.isQueuesEmpty())) {
                //     centerSchedulersBusyMap.put(interScheduler.getId(), true);
                //     send(this, loadBalancer.getLoadBalanceCostTime(), CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
                // }
            }
        }
    }
    ...
    /**
     * Start the presolve process of an inter-scheduler.
     *
     * @param evt the event
     */
    private void processInterPresolveBegin(SimEvent evt) {
        if (evt.getData() instanceof InterScheduler interScheduler) {
            LOGGER.info("{}: {}'s {} starts presolve.", getSimulation().clockStr(), getName(), interScheduler.getName());
            interScheduler.presolve();
            send(this, interScheduler.getPresolveTime(), CloudSimTag.INTER_PRESOLVE_END, interScheduler);
        }
    }

    /**
     * Start the inter-scheduling process of an inter-scheduler.
     * If pipeline is enabled, the presolve process will be started parallelly.
     *
     * @param evt the event
     */
    private void processInterPresolveEnd(SimEvent evt) {
        if (evt.getData() instanceof InterScheduler interScheduler) {
            LOGGER.info("{}: {}'s {} ends presolve.", getSimulation().clockStr(), getName(), interScheduler.getName());

            Map<Integer, Boolean> centerSchedulersPresolveBusyMap = getSimulation().getCollaborationManager().getCenterSchedulersPresolveBusyMap(interScheduler.getCollaborationId());
            Map<Integer, Boolean> centerSchedulersScheduleBusyMap = getSimulation().getCollaborationManager().getCenterSchedulersScheduleBusyMap(interScheduler.getCollaborationId());

            if(!interScheduler.isEnablePipeline()) {
                send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
            } else {
                if(!interScheduler.isPresolveQueuesEmpty()) {
                    send(this, 0, CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
                } else {
                    centerSchedulersPresolveBusyMap.put(interScheduler.getId(), false);
                }
                if ((!centerSchedulersScheduleBusyMap.containsKey(interScheduler.getId()) || !centerSchedulersScheduleBusyMap.get(interScheduler.getId()))
                            && !interScheduler.isScheduleQueuesEmpty()) {
                    send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
                    centerSchedulersScheduleBusyMap.put(interScheduler.getId(), true);
                }
            }

            // send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
            // if(interScheduler.isEnablePipeline() && !interScheduler.isQueuesEmpty()) {
            //     send(this, 0, CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
            // }
        }
    }
    ...
    private void startCenterInterScheduling(InterScheduler interScheduler) {
        Map<Integer, Boolean> centerSchedulersBusyMap = getSimulation().getCollaborationManager().getCenterSchedulersBusyMap(interScheduler.getCollaborationId());
        Map<Integer, Boolean> centerSchedulersScheduleBusyMap = getSimulation().getCollaborationManager().getCenterSchedulersScheduleBusyMap(interScheduler.getCollaborationId());
        
        if(!interScheduler.isEnablePipeline()) {
            if (interScheduler.isQueuesEmpty()) {
                centerSchedulersBusyMap.put(interScheduler.getId(), false);
            } else {
                send(this, 0, CloudSimTag.INTER_PRESOLVE_BEGIN, interScheduler);
            }
        } else {
            if(interScheduler.isScheduleQueuesEmpty()) {
                centerSchedulersScheduleBusyMap.put(interScheduler.getId(), false);
            } else {
                send(this, 0, CloudSimTag.INTER_SCHEDULE_BEGIN, interScheduler);
            }
        }
    ...
    private boolean allocateBwForGroup(InstanceGroup instanceGroup, Datacenter receivedDatacenter) {
        ...
        for (InstanceGroup dst : dstInstanceGroups) {
            if (dst.getReceiveDatacenter() != Datacenter.NULL && !dst.getReceiveDatacenter().isFailureCluster()) {
        ...
        for (InstanceGroup src : srcInstanceGroups) {
            if (src.getReceiveDatacenter() != Datacenter.NULL && !src.getReceiveDatacenter().isFailureCluster()) {
    ...
    private void processUserRequestFail(SimEvent evt) {
        if (evt.getData() instanceof Set<?> userRequestsTmp) {
            if (!userRequestsTmp.isEmpty() && userRequestsTmp.iterator().next() instanceof UserRequest) {
                ...
                List<UserRequest> failedUserRequests = new ArrayList<>(userRequests);
                failedUserRequests.removeIf(userRequest -> userRequest.getState() == UserRequest.FAILED);
                for (UserRequest userRequest : userRequests) {
                    processAUserRequestFail(userRequest);
                }
                
                if(this.getFailureCluster() != Datacenter.NULL) {  // calculate the failure penalty through FailureCluster

                    updateUserRequestsForFailureCluster(failedUserRequests);

                    sendSlowly(this.getFailureCluster(), 0, CloudSimTag.USER_REQUEST_SEND, failedUserRequests);
                }
            }
        }
    }

    private void updateUserRequestsForFailureCluster(List<UserRequest> userRequests) {
        for (UserRequest userRequest : userRequests) {
            updateUserRequestForFailureCluster(userRequest);
        }
    }

    private void updateUserRequestForFailureCluster(UserRequest userRequest) {
        int FAILURE_RETRY_MAX_NUM = 10;
        for (InstanceGroup instanceGroup : userRequest.getInstanceGroups()) {
            instanceGroup.setRetryNum(0);
            instanceGroup.setRetryMaxNum(FAILURE_RETRY_MAX_NUM);
            for (Instance instance : instanceGroup.getInstances()) {
                instance.setRetryNum(0);
                instance.setRetryMaxNum(FAILURE_RETRY_MAX_NUM);
            }
        }
    }
    ...
    private void releaseBwForFailedUserRequest(UserRequest userRequest) {
        ...
        for (InstanceGroupEdge allocateEdge : allocateEdges) {
            ...
            if (src != Datacenter.NULL && !src.isFailureCluster() && dest != Datacenter.NULL && !dest.isFailureCluster()) {
