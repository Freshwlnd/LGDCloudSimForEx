...
public abstract class CloudSimEntity implements SimEntity {
    ...
    /**
     * Get the max network delay.
     * @return the max network delay
     */
    private double getMaxNetworkDelay() {
        return getSimulation().getNetworkTopology().getMaxDelay();
    }
    ...
    /**
     * Sends an event from one entity to another now slower than the default network delay.
     * @param dest the destination entity
     * @param delay how long from now the event should be executed
     * @param tag a user-defined number representing the type of event, @see {@link SimEvent#getTag()}
     * @param data a reference to an object to be sent with the event
     */
    protected void sendSlowly(final SimEntity dest, double delay, final int tag, final Object data) {
        requireNonNull(dest);
        if (dest.getId() < 0) {
            LOGGER.error("{}.send(): invalid entity id {} for {}", getName(), dest.getId(), dest);
            return;
