...
public class UserRequestManagerCsv implements UserRequestManager {
    ...
    /**
     * The flag of whether the edge of instance group graph is homogeneous.
     */
    private boolean isGroupEdgeHomogeneous = false;

    /**
     * The flag of whether the access delay of instance group is homogeneous.
     */
    private boolean isAccessDelayHomogeneous = false;
        ...
        try (CSVParser csvParser = new CSVParser(new FileReader(csvFile), csvFormat)) {
            for (CSVRecord csvRecord : csvParser) {
                ...
                switch (title) {
                    ...
                    case "isGroupEdgeHomogeneous" -> this.isGroupEdgeHomogeneous = Boolean.parseBoolean(csvRecord.get(1));
                    case "isAccessDelayHomogeneous" -> this.isAccessDelayHomogeneous = Boolean.parseBoolean(csvRecord.get(1));
    ...
    private InstanceGroup generateAnInstanceGroup() {
        ...
        int retryTimes = Math.max(0, getExpectedValue(GroupRetryTimes, GroupRetryTimesMin, GroupRetryTimesMax));
    ...
    private InstanceGroupGraph generateAnInstanceGroupGraph(List<InstanceGroup> instanceGroups) {
        ...
        if (random.nextDouble() < GroupBwPercent) {
            commonBw = getExpectedValue(GroupBw, GroupBwMin, GroupBwMax);
            commonBw = BigDecimal.valueOf(commonBw).setScale(2, RoundingMode.HALF_UP).doubleValue();
        }
        double commonDelay = Double.MAX_VALUE;
        if (random.nextDouble() < GroupDelayPercent) {
            commonDelay = getExpectedValue(GroupDelay, GroupDelayMin, GroupDelayMax);
            commonDelay = BigDecimal.valueOf(commonDelay).setScale(2, RoundingMode.HALF_UP).doubleValue();
        }
    ...
    private UserRequest generateAUserRequest() {
        ...
        for (int i = 0; i < groupNum; i++) {
            ...
            if(isAccessDelayHomogeneous){
                instanceGroups.get(i).setAccessLatency(instanceGroups.get(0).getAccessLatency());
            }
        }
